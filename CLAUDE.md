...
Unity 6 URP 프로젝트. NOMANUAL 0(1편 프리퀄)이고 지금은 0일차 MVP를 만든다. 단계별 계획은 `Roosevelt_Doc/01_기획/0일차_기획.md` 7장에 있다. 게임플레이 코드는 `Assets/Scripts/` 아래 Player(이동·`InputLock`), Interaction(`IInteractable`·`PlayerInteraction`·이동용 문 `TransitionDoor`), UI(픽셀 폰트 HUD), Environment(문), Presentation(암전 전환 `ScreenTransition`), Flow(`GameFlow`, 빌드 목록 밖 씬도 여는 `SceneLoader`, F1 디버그 패널), Creatures, Core(`Roosevelt.Core` asmdef: GameState·플래그·저장)로 나뉜다. `Assets/TutorialInfo/`와 `Assets/Readme.asset`은 템플릿 안내용이라 지워도 된다.

세계관은 전작 NOMANUAL의 Rhizome Solution(R.S)을 잇는다. 설정 문서는 `Roosevelt_Doc/02_세계관/세계관_리좀솔루션.md`이고, 스토리·기획 작업 전에 읽는다.

## 폴더 구분

- `D:\04_Project Files\Roosevelt\Roosevelt\` (이 저장소): 개발 루트. 게임 코드와 에셋은 여기에만 둔다. origin은 `https://github.com/w8err/Roosevelt`, 브랜치는 `main`.
- `Roosevelt_Doc/` (이 저장소 안): 기획서·분석·선별 이미지를 관리하는 Obsidian vault. Unity가 `Assets/` 밖은 임포트하지 않으므로 여기 두어도 프로젝트에 영향이 없다.
